        ...
        public string gmGetMstID(string MstType, string MstDesc)
        {
            ...
            switch (MstType)
            {
                ...
                case "V":
                    ...
                    SQL = "Select Max(Right(customer_id,4))+1 As MaxID From customerData Where Right(Left(customer_id,1),1)= 'V'";

                    break;
                case "A":

                    SQL = "Select Max(Right(bill_no,4))+1 As MaxID From paymentDetails Where Right(Left(bill_no,1),1)= 'A'";
                    break;

                case "S":

                    SQL = "Select Max(Right(bill_no,4))+1 As MaxID From paymentDetails Where Right(Left(bill_no,1),1)= 'S'";

                    break;
                case "C":

                    SQL = "Select Max(Right(customer_id,4))+1 As MaxID From customerData Where Right(Left(customer_id,1),1)= 'C'";
                    break;

                case "M":

                    SQL = "Select Max(Right(bill_no,4))+1 As MaxID From paymentDetails Where Right(Left(bill_no,1),1)= 'M'";
                    break;
